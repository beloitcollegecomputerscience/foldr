...
/**
 * A Jreality tool, which animates a shape an input distance over a set amount
 * of frames. Currently will only work for translation.
 * 
 * @author lizlevy
 * 
 */
public class AnimateMovement extends AbstractTool {
	...
	// occur in
	static int totalFramesForAnimation = 400;
	...
	public void setEndPoints(Shape newShapeToMove, double[] newEndPoints) {
		...
		intervalToMoveX = distanceToMoveX / (double) totalFramesForAnimation;
		intervalToMoveY = distanceToMoveY / (double) totalFramesForAnimation;
		intervalToMoveZ = distanceToMoveZ / (double) totalFramesForAnimation;
	}
	...
	public void perform(ToolContext tc) {
		...
		if (currentFrame == totalFramesForAnimation) {
			double errorX = sgcToMove.getTransformation().getMatrix()[3]
					- (originalX + distanceToMoveX);
			double errorY = sgcToMove.getTransformation().getMatrix()[7]
					- (originalY + distanceToMoveY);
			double errorZ = sgcToMove.getTransformation().getMatrix()[11]
					- (originalZ + distanceToMoveZ);
			...
			MatrixBuilder
					.euclidean()
					.translate(originalX + newX, originalY + newY,
							originalZ + newZ)
					.rotate(shapeToMove.rotationList[0],
							shapeToMove.rotationList[1],
							shapeToMove.rotationList[2][0]).assignTo(sgcToMove);
